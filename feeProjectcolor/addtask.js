...
function addTask() {
  ...
  if (taskName.trim() !== "") {
      var task = {
          ...
          date: taskDate,
          time: taskTime,
          color: selectedColor // Add selected color to the task object
      };
      ...
      var tasksString = tasksArray
          .map(function(task) {
              return (
                  task.name + "|" + task.description + "|" + task.date + "|" + task.time + "|" + task.color // Include color in the stored task data
              );
          })
          .join(",");
